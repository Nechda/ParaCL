...
class JIT {
  public:
    static Expected<std::unique_ptr<JIT>> Create() {
        ...
        auto ProcessSymbolsSearchGenerator = DynamicLibrarySearchGenerator::GetForCurrentProcess(DL->getGlobalPrefix());
        if (!ProcessSymbolsSearchGenerator)
            return ProcessSymbolsSearchGenerator.takeError();

        std::unique_ptr<JIT> jit(
            new JIT(std::move(ES), std::move(*DL), std::move(*JTMB), std::move(*ProcessSymbolsSearchGenerator)));
        return std::move(jit);
    }
  ...
  private:
    /* Constructor */
    JIT(std::unique_ptr<ExecutionSession> ES, DataLayout DL, orc::JITTargetMachineBuilder JTMB,
        std::unique_ptr<DynamicLibrarySearchGenerator> ProcessSymbolsGenerator)
        : ES(std::move(ES)), DL(std::move(DL)), Mangle(*this->ES, this->DL), MainJD(this->ES->createJITDylib("<main>")),
