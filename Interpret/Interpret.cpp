...
#define ASSERT(expr_) if(!(expr_)) {printf("Assert failed: " #expr_); exit(1);}
...
struct ExecutionVisitor : public AST::IVisitor {
    ...
    void visit(AST::BinaryInst& node) {
        ...
        switch (node.op_)
        {
            case AST::BinaryInst::OP::ADD: value_stack_.push(lhs_value + rhs_value); break;
            case AST::BinaryInst::OP::SUB: value_stack_.push(lhs_value - rhs_value); break;
            case AST::BinaryInst::OP::MUL: value_stack_.push(lhs_value * rhs_value); break;
            case AST::BinaryInst::OP::DIV: value_stack_.push(lhs_value / rhs_value); break;
            default: break;
        }
    }
    ...
    void visit(AST::BranchInst& node) {
        cur_bb_ = node.bb_next_;
    }

    void visit(AST::Variable& node) {
        value_stack_.push(variables_[node.name_]);
    }
